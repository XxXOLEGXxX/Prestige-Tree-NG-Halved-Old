let modInfo = {
	...
	id: "ptr/",
	author: "Jacorb (Modified by Holy Broly#0530)",
	pointsName: "points",
...
function getPointGen() {
	...
	if (hasMilestone("omega", 7)) gain = new Decimal(100)
	if (hasMilestone("omega", 8)) gain = gain.mul(100)
	if (hasMilestone("omega", 9)) gain = gain.mul(100)
	...
	if (hasAchievement("a", 21)) gain = gain.times(1.05);
	if (hasAchievement("a", 31)) gain = gain.times(1.25);
	...
	if (hasMilestone("omega", 0) && !hasMilestone("omega", 4)) gain = gain.mul(player.p.upgrades.length+1)
	if (hasMilestone("omega", 4)) gain = gain.mul(new Decimal(2).pow(player.p.upgrades.length))
	if (hasMilestone("omega", 1)) gain = gain.mul((player.b.upgrades.length+1)*(player.g.upgrades.length+1))
	if (hasMilestone("omega", 6)) gain = gain.mul(new Decimal(player.g.milestones.length).add(1).pow(2).mul(new Decimal(player.b.milestones.length).add(1).pow(2)))
	return gain.pow(tmp.omega.effect)
}
