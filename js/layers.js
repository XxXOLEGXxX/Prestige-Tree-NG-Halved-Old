addLayer("p", {
        ...
        gainMult() { // Calculate the multiplier for main currency from bonuses
            mult = new Decimal(1)
			if (hasAchievement("a", 13)) mult = mult.times(1.05);
			if (hasUpgrade("p", 12)&&hasMilestone("omega", 5)) mult = mult.times(upgradeEffect("p", 12));
			if (hasUpgrade("p", 21)) mult = mult.times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1e50:1.4);
			...
			if (hasMilestone("omega", 0)&&hasMilestone("omega", 2)&&!hasMilestone("omega", 4)) mult = mult.mul(player.p.upgrades.length+1)
			if (hasMilestone("omega", 0)&&hasMilestone("omega", 2)&&hasMilestone("omega", 4)) mult = mult.mul(new Decimal(2).pow(player.p.upgrades.length))
			if (hasMilestone("omega", 1)&&hasMilestone("omega", 3)) mult = mult.mul((player.b.upgrades.length+1)*(player.g.upgrades.length+1))
            return mult
        },
		...
		passiveGeneration() { return (hasMilestone("g", 1)&&player.ma.current!="p")?0.5:0 },
		...
		upgrades: {
			...
			11: {
				title: "Begin",
				description() {return hasMilestone("omega", 9) ? "Generate 1,000,000 Points every second." : hasMilestone("omega", 8) ? "Generate 10,000 Points every second." : hasMilestone("omega", 7) ? "Generate 100 Points every second." : "Generate 0.5 Point every second."},
			...
			12: {
				...
				effect() {
					let eff = player.p.points.plus(1).pow(0.25);
					if (hasUpgrade("g", 14)) eff = eff.pow(1.25);
				...
				formula() { 
					let exp = new Decimal(0.25*(hasUpgrade("g", 14)?1.5:1)*(hasUpgrade("g", 24)?1.4666667:1));
			...
			13: {
				...
				effect() { 
					let eff = player.points.plus(0.5).log(5).pow(0.375).plus(0.5);
				...
				formula() { 
					...
					return "(log5(x+0.5)^0.375+0.5)"+(exp.gt(0.5)?("^"+format(exp)):"")
				},
			},
			14: {
			...
			21: {
				title: "More Prestige",
				description() { return "Prestige Point gain is increased by "+(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?"1e52":"40")+"%." },
			...
			22: {
				...
				effect() {
					let eff = Decimal.pow(1.2, player.p.upgrades.length);
				...
				formula() { 
					...
					return exp.gt(1)?("(1.4^x)^"+format(exp)):"1.2^x" 
				},
			},
			23: {
				...
				effect() {
					let eff = player.points.plus(0.5).log(5).pow(1/6).plus(0.5);
					if (hasUpgrade("p", 33)) eff = eff.pow(upgradeEffect("p", 33));
				...
				formula() { 
					...
					return exp.gt(1)?("(log5(x+0.5)^(1/6)+0.5)^"+format(exp)):"log5(x+0.5)^(1/6)+0.5"
				},
			},
			24: {
...
addLayer("b", {
		...
		upgrades: {
			...
			11: {
				...
				effect() { 
					let ret = player.b.best.root(1.5).plus(0.5);
			...
			12: {
				...
				effect() {
					let ret = player.g.points.add(0.5).log(5).root(1.5).div(6).times(hasUpgrade("e", 14)?upgradeEffect("e", 14):1);
					if (hasUpgrade("b", 14) && player.i.buyables[12].gte(1)) ret = ret.pow(upgradeEffect("b", 14));
					return ret;
				},
				...
				formula() { 
					let exp = new Decimal(1);
					if (hasUpgrade("b", 14) && player.i.buyables[12].gte(1)) exp = exp.times(upgradeEffect("b", 14));
					let f = "yroot1.5(log5(x+0.5))"+(hasUpgrade("e", 14)?("*"+format(upgradeEffect("e", 14).div(3))):"/6") 
			...
			13: {
				...
				effect() { 
					let ret = player.p.total.add(0.5).log(5).add(0.5).log(5).div(6).times(hasUpgrade("e", 14)?upgradeEffect("e", 14):1) 
					if (hasUpgrade("b", 14) && player.i.buyables[12].gte(1)) ret = ret.pow(upgradeEffect("b", 14));
					return ret;
				},
				...
				formula() { 
					let exp = new Decimal(1)
					if (hasUpgrade("b", 14) && player.i.buyables[12].gte(1)) exp = exp.times(upgradeEffect("b", 14));
					let f = "log5(log5(x+0.5)+0.5)"+(hasUpgrade("e", 14)?("*"+format(upgradeEffect("e", 14).div(3))):"/6") 
					if (exp.gt(1)) f = "("+f+")^"+format(exp);
					return f;
				},
			},
			...
			21: {
				title: "Gen Z^1.5",
				description: "Exponent Generator Power effect by 1.5.",
			...
			22: {
				title: "Up to the Tenth Floor",
				description: "Raise the Generator Power effect ^1.1.",
			...
			23: {
				...
				effect() { 
					let ret = player.points.add(0.5).log(5).add(0.5).pow(1.6);
				...
				formula() { return "(log5(x+0.5)+0.5)^"+(player.s.unlocked?format(buyableEffect("s", 14).times(3.2).times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1.5:1)):"1.6") },
			},
			24: {
...
addLayer("g", {
		...
		effect() {
			...
			let eff = new Decimal(0.5).mul(this.effBase().pow(player.g.points.sub(1).plus(tmp.sg.spectralTotal)).max(0));
			if (player.g.points.eq(0)) eff = new Decimal(0)
			if (player.g.points.eq(1)) eff = new Decimal(0.5)
		...
		powerExp() {
			...
			if (hasUpgrade("b", 21)) exp = exp.times(1.5);
			if (hasUpgrade("b", 22)) exp = exp.times(1.1);
		...
		milestones: {
			...
			1: {
				...
				effectDescription: "You gain 50% of Prestige Point gain every second.",
			},
			2: {
		...
		upgrades: {
			...
			11: {
				...
				effect() { return player.g.best.root(1.5).plus(0.5).pow(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?5e5:1) },
				unlocked() { return player.g.unlocked },
				effectDisplay() { return format(tmp.g.upgrades[11].effect)+"x" },
				formula() { return ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?"(x+1)^250,000":"yroot1.5(x)+0.5" },
			},
			12: {
				title: "I Need More!",
				...
				effect() { 
					let ret = player.b.points.add(0.5).log(5).root(1.5).div(6).times(hasUpgrade("e", 14)?upgradeEffect("e", 14):1);
				...
				formula() { 
					let m = new Decimal(hasUpgrade("e", 14)?upgradeEffect("e", 14):1).div(6)
					if (hasUpgrade("s", 24)) m = upgradeEffect("s", 24).times(m);
					return "yroot1.5(log5(x+0.5))"+(m.eq(1)?"":(m.gt(1)?("*"+format(m)):("/"+format(m.pow(-1)))));
				},
			},
			13: {
				...
				effect() { 
					let ret = player.p.best.add(0.5).log(5).add(0.5).log(5).div(6).times(hasUpgrade("e", 14)?upgradeEffect("e", 14):1);
					if (hasUpgrade("s", 24)) ret = ret.times(upgradeEffect("s", 24));
					return ret;
				},
				...
				formula() { 
					let m = new Decimal(hasUpgrade("e", 14)?upgradeEffect("e", 14):1).div(6)
					if (hasUpgrade("s", 24)) m = upgradeEffect("s", 24).times(m);
					return "log5(log5(x+0.5)+0.5)"+(m.eq(1)?"":(m.gt(1)?("*"+format(m)):("/"+format(m.pow(-1)))));
				},
			},
			14: {
				title: "Boost the Boost",
				description() { return "<b>Prestige Boost</b> is raised to the power of 1.25." },
			...
			15: {
				...
				effect() { 
					let eff = player.g.points.root(1.5).add(0.5);
					if (eff.gte(400)) eff = eff.cbrt().times(Math.pow(400, 2/3))
					return eff;
				},
				unlocked() { return hasUpgrade("g", 13) },
				effectDisplay() { return "^"+format(tmp.g.upgrades[15].effect) },
				formula() { return upgradeEffect("g", 15).gte(400)?"((x+1)^(1/6))*(400^(2/3))":"yroot1.5(x)+0.5" },
			},
			21: {
...
addLayer("s", {
		...
		space() {
			...
			if (hasAchievement("a", 53)) space = space.plus(1);
		...
		buildingPower() {
			...
			if (hasAchievement("a", 103)) pow = pow.plus(.05);
...
addLayer("h", {
		...
		challenges: {
			...
			31: {
				...
				completionLimit() { 
					let lim = 5
					if (hasAchievement("a", 71)) lim += 5;
					if (hasAchievement("a", 74)) lim += 5;
		...
		buyables: {
			...
			11: {
				...
				costBase() {
					...
					if (hasAchievement("a", 101)) base = base.sub(.1);
...
addLayer("hn", {
		...
		gainMult() {
			...
			if (hasAchievement("a", 91)) mult = mult.times(1.05);
...
addLayer("hs", {
		...
		buildingPower() {
			...
			if (hasAchievement("a", 113)) pow = pow.plus(.05);
...
addLayer("ma", {
        ...
        gainMult() { // Calculate the multiplier for main currency from bonuses
            mult = new Decimal(1)
			if (hasAchievement("a", 131)) mult = mult.div(1.05);
			if (hasAchievement("a", 95)) mult = mult.div(1.075);
			if (hasAchievement("a", 134)) mult = mult.times(Decimal.pow(.9999625, player.ps.points));
            return mult
        },
        gainExp() { // Calculate the exponent on main currency from bonuses
            return new Decimal(1)
...
addLayer("ge", {
		...
		buyables: {
			...
			11: {
				...
				costDiv() {
					let div = new Decimal(1);
					if (hasAchievement("a", 124)) div = div.times(1.5);
					return div;
				},
				free() {
					let free = new Decimal(0);
					if (hasAchievement("a", 132)) free = free.plus(1);
					return free;
				},
				power() {
					let pow = new Decimal(1);
					if (hasAchievement("a", 124)) pow = pow.times(1.1);
					return pow;
				},
				cost(x=player[this.layer].buyables[this.id]) { // cost for buying xth buyable, can be an object if there are multiple currencies
		...
		clickables: {
			...
			12: {
				...
				effectPer() { return Decimal.add(6, tmp.ge.buyables[11].effect).times(hasAchievement("a", 123)?2:1) },
				effect() { return Decimal.pow(tmp.ge.clickables[this.id].effectPer, player.ge.clickables[this.id]) },
...
addLayer("mc", {
		...
		clickables: {
			...
			21: {
				...
				effExp() { return hasAchievement("a", 133)?1.5:1 },
		...
		buyables: {
			...
			11: {
				...
				costDiv() { return new Decimal(hasAchievement("a", 132)?3.5:1) },
				cost(x=player[this.layer].buyables[this.id]) { // cost for buying xth buyable, can be an object if there are multiple currencies
                    return x.div(10).plus(0.5).div(tmp[this.layer].buyables[this.id].costDiv).ceil();
                },
				buffExp() { return hasAchievement("a", 132)?12.5:5 },
...
addLayer("a", {
        ...
        achievements: {
			...
			13: {
				...
				tooltip: "Purchase 3 Prestige Upgrades. Reward: Gain 5% more Prestige Points.",
			...
			21: {
				...
				tooltip: "Perform a Row 2 reset. Reward: Generate Points 5% faster, and unlock 3 new Prestige Upgrades.",
			...
			31: {
				...
				tooltip: "Perform a Row 3 reset. Reward: Generate Points 25% faster, and Boosters/Generators don't increase each other's requirements.",
			...
			53: {
				...
				tooltip: "Perform a Super-Generator reset. Reward: Get 1 extra Space.",
			...
			71: {
				...
				tooltip: 'Perform a Row 5 reset. Reward: Always have all milestones of Row 2, 3, and 4, and you can complete "Timeless" 5 more times.',
			...
			74: {
				...
				tooltip: 'Reach 1e100 Balance Energy. Reward: You can complete "Timeless" 5 more times, and the "Option D" effect also affects Magic & Balance Energy gain.',
			...
			91: {
				...
				tooltip: "Unlock Nebula or Hyperspace. Reward: Gain 5% more Honour.",
			...
			94: {
				...
				done() { return player.h.challenges[31]>=15 && player.h.challenges[32]>=5 },
				tooltip: 'Complete "Timeless" 15 times and "Option D" 5 times.',
			...
			95: {
				...
				tooltip: "Master either Nebula or Hyperspace. Mastery is 7.5% cheaper.",
				image: "images/achs/95.png",
			},
			101: {
				name: "Realm of The Impossible",
				done() { return player.q.points.gte("e1e6") },
				tooltip: "Reach e1,000,000 Quirks. Reward: The Quirk Layer cost base is reduced by 0.1.",
				image: "images/achs/101.png",
			},
			102: {
				name: "We're Not Beyond This?",
				done() { return inChallenge("h", 31) && player.h.challenges[31]>=15 && player.points.gte("e2e7") },
				tooltip: 'Reach e20,000,000 Points while in the "Timeless" hindrance (which must be completed at least 15 times).',
				image: "images/achs/102.png",
			},
			103: {
				name: "One Billion Zeros",
				done() { return player.points.gte("e1e9") },
				tooltip: "Reach e1e9 Points. Reward: Add 5% to Space Building Power.",
			...
			113: {
				...
				tooltip: "Reach e1e12 Points. Reward: Add 5% to Hyper Building Power.",
			...
			123: {
				...
				tooltip: "Reach 1.2e34 J of Kinetic Energy. Reward: The Kinetic Energy Gear Upgrade's base is doubled.",
				image: "images/achs/123.png",
			},
			124: {
				name: "The Perfect Being",
				done() { return player.hn.points.gte("ee6") },
				tooltip: "Reach e1,000,000 Honour. Reward: Gear Evolution requires 1.5x less Rotations, and is 10% stronger.",
			...
			131: {
				...
				tooltip: "Unlock Machines. Reward: Mastery is 5% cheaper.",
				image: "images/achs/131.png",
			},
			132: {
				name: "God is a Turtle",
				done() { return player.mc.buyables[11].gte(200) },
				tooltip: "Reach a Shell size of at least 200m. Reward: Shell Expansion's buff is raised ^2.5, its cost is divided by 3.5, & you get 1 free Gear Evolutions.",
				image: "images/achs/132.png",
			},
			133: {
				name: "Breaching the Barriers",
				done() { return player.mc.mechEn.gte("1e375") },
				tooltip: "Reach 1e375 Mech-Energy. Reward: You can have 2 parts of The Motherboard active at once, Northbridge's effect is 1.5th'd, and there is a new Gear Upgrade.",
				image: "images/achs/133.png",
			},
			134: {
				name: "Innermost Desire",
				done() { return player.mc.upgrades.includes(11) },
				tooltip() { return "Unlock The Core. Reward: Mastery is 0.00375% cheaper for every Phantom Soul you have (Currently: "+format(Decimal.sub(1, Decimal.pow(.9999625, player.ps.points)).times(100))+"% cheaper)" },
...
addLayer("omega", {
        name: "omega", // This is optional, only used in a few places, If absent it just uses the layer id.
        symbol: "Ω", // This appears on the layer's node. Default is the id with the first letter capitalized
        color: "#CCCCCC",
        requires() { return player.omega.points.gte(11) ? new Decimal(Infinity) : player.omega.points.gte(10) ? new Decimal(9.99e41) : player.omega.points.gte(9) ? new Decimal(1e39) : player.omega.points.gte(8) ? new Decimal(3.6e34) : player.omega.points.gte(7) ? new Decimal(4.2e24) : player.omega.points.gte(6) ? new Decimal(1.2e18) : player.omega.points.gte(5) ? new Decimal(120000000) : player.omega.points.gte(4) ? new Decimal(12000000) : player.omega.points.gte(3) ? new Decimal(2100000) : player.omega.points.gte(2) ? new Decimal(200000) : player.omega.points.gte(1) ? new Decimal(1000) : new Decimal(8) }, // Can be a function that takes requirement increases into account
        resource: "omega levels", // Name of prestige currency
        ...
        hotkeys: [
            {key: "o", description: "Press O to perform an omega reset", onPress(){if (canReset(this.layer)) doReset(this.layer)}},
        ],
        layerShown(){return player.omega.unlocked || player.p.upgrades.length >= 3},
		effect() {
			return player.omega.points.div(100).add(1);
		},
		effectDescription() {
			return "which are boosting Point generation by "+format(tmp.omega.effect)+"^"+(tmp.nerdMode?("\n (+0.01^ each)"):"")
		},
		startData() { return {
			unlocked: false,
			points: new Decimal(0),
		...
		milestones: {
			0: {
				requirementDescription: "1 Omega Level",
				done() { return player.omega.best.gte(1) },
				effectDescription() { return "Point gain is "+formatWhole(hasMilestone("omega", 4) ? new Decimal(2).pow(player.p.upgrades.length) : player.p.upgrades.length+1)+"x faster based on your Prestige Upgrades bought, regardless of ANY non-Omega effect whatsoever." },
			},
			1: {
				requirementDescription: "2 Omega Levels",
				done() { return player.omega.best.gte(2) },
				effectDescription() { return "Point gain is "+formatWhole((player.g.upgrades.length+1)*(player.b.upgrades.length+1))+"x faster based on your Booster Upgrades and Generator Upgrades bought, regardless of ANY non-Omega effect whatsoever." },
			},
			2: {
				requirementDescription: "3 Omega Levels",
				done() { return player.omega.best.gte(3) },
				effectDescription() { return "1 Omega Level milestone boosts Prestige Point gain as well." },
			},
			3: {
				requirementDescription: "4 Omega Levels",
				done() { return player.omega.best.gte(4) },
				effectDescription() { return "2 Omega Levels milestone boosts Prestige Point gain as well." },
			},
			4: {
				requirementDescription: "5 Omega Levels",
				done() { return player.omega.best.gte(5) },
				effectDescription() { return "1 Omega Level milestone uses better formula." },
			},
			5: {
				requirementDescription: "6 Omega Levels",
				done() { return player.omega.best.gte(6) },
				effectDescription() { return "Prestige Boost boosts Prestige Point gain." },
			},
			6: {
				requirementDescription: "7 Omega Levels",
				done() { return player.omega.best.gte(7) },
				effectDescription() { return "Point gain is "+formatWhole(new Decimal(player.g.milestones.length).add(1).pow(2).mul(new Decimal(player.b.milestones.length).add(1).pow(2)))+"x faster based on your Booster Upgrades and Generator Upgrades bought, regardless of ANY non-Omega effect whatsoever." },
			},
			7: {
				requirementDescription: "8 Omega Levels",
				done() { return player.omega.best.gte(8) },
				effectDescription() { return "Begin now generates 100 Points every second, as if it always has been like that." },
			},
			8: {
				requirementDescription: "9 Omega Levels",
				done() { return player.omega.best.gte(9) },
				effectDescription() { return "Exponents Begin by 2." },
			},
			9: {
				requirementDescription: "10 Omega Levels",
				done() { return player.omega.best.gte(10) },
				effectDescription() { return "Begin is expontented by 3 instead." },
			},
			10: {
				requirementDescription: "11 Omega Levels",
				done() { return player.omega.best.gte(11) },
				effectDescription() { return "I'm out of motivations. [ENDGAME]" },
			},
		},
		doReset(resettingLayer) {
